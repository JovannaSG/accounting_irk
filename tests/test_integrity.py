...
def test_identity_ignores_inactive_negative_rows():
    """Неактивные строки (без положительных остатков) не участвуют в расчёте rate."""
    good_row = {
        "Период": "2026-01-31", "Счет": "51", "Субконто": "-", "Тип": "A",
        "НачалоДебет": 0, "НачалоКредит": 0,
        "ОборотДебет": 100, "ОборотКредит": 100,
        "КонецДебет": 100, "КонецКредит": 100, "Договор": "-",
    }
    bad_negative_row = {
        "Период": "2026-01-31", "Счет": "66", "Субконто": "-", "Тип": "P",
        "НачалоДебет": -50, "НачалоКредит": -50,
        "ОборотДебет": -90, "ОборотКредит": -50,
        "КонецДебет": -50, "КонецКредит": -50, "Договор": "-",
    }
    df = pd.DataFrame([good_row] * 10 + [bad_negative_row] * 2)
    assert check_bookkeeping_identity(df) == []
