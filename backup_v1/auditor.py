...
REQUIRED_OSV = ["Счет", "Тип", "КонецДебет", "КонецКредит"]
NUMERIC_OSV = ["НачалоДебет", "НачалоКредит", "ОборотДебет",
               "ОборотКредит", "КонецДебет", "КонецКредит"]
VALID_TYPES = {"A", "P", "AP"}

DEFAULT_CLOSING_ACCOUNTS = ["25", "26", "44", "90", "91", "99"]
SETTLEMENT_GROUPS = ["60", "62", "76"]

DETAIL_COLUMNS = ["Проверка", "Уровень", "Период", "Счет", "Субконто",
...
def normalize_balances(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Субконто"] = (df["Субконто"].fillna("-") if "Субконто" in df.columns
                      else pd.Series("-", index=df.index)).astype(str)
    df["Период"] = (df["Период"].fillna("") if "Период" in df.columns
    ...
    order = ["Период", "Счет", "Субконто", "Тип", "НачалоДебет", "НачалоКредит",
             "ОборотДебет", "ОборотКредит", "КонецДебет", "КонецКредит"]
...
def normalize_documents(df: pd.DataFrame) -> pd.DataFrame:
    ...
    kind = {
        ...
        "оплата": "оплата", "платеж": "оплата", "платёж": "оплата", "поступление": "оплата",
...
class AutoAuditor1C:
    ...
    def __init__(self, balances_df: pd.DataFrame,
                 ...
                 closing_accounts: Optional[list] = None) -> None:
        ...
        self.closing_accounts = {str(a).split(".")[0] for a in (closing_accounts or DEFAULT_CLOSING_ACCOUNTS)}
    ...
    def _add(self, level: str, title: str, data: pd.DataFrame) -> None:
        ...
        self.errors.append({"level": level, "title": title, "data": data, "amount": amount})
    ...
    def check_unclosed_month_end(self) -> None:
        b = self.balances
        closing = b[(b["Счет"].map(account_group).isin(self.closing_accounts)) &
                    ((b["КонецДебет"] > 0) | (b["КонецКредит"] > 0))]
        ...
        stuck = b2[((b2["КонецДебет"] > 0) | (b2["КонецКредит"] > 0)) &
                   (b2["КонецДебет"] == prev_d) &
                   (b2["КонецКредит"] == prev_k) &
                   prev_d.notna()]
        stuck = stuck.copy()
        stuck["Комментарий"] = "Сальдо не меняется между периодами (зависший остаток)"
        self._add("warning", "Зависшее сальдо (не меняется между периодами)", stuck)

    # ------------------------------------------------------------------ #
    # 4.4 Счет 000
    # ------------------------------------------------------------------ #
    def check_account_000(self) -> None:
        acc = self.balances[(self.balances["Счет"].map(account_group) == "000") &
    ...
    def check_unclosed_settlements(self) -> None:
        if self.documents is None:
            ...
            dup = sett[(sett["Субконто"] != "-") &
                       (sett["КонецДебет"] > 0) & (sett["КонецКредит"] > 0)]
            dup = dup.copy()
        ...
        res = pd.DataFrame(rows, columns=["Период", "Счет", "Субконто",
    ...
    def summary_df(self) -> pd.DataFrame:
        rows = [{"Проверка": e["title"], "Уровень": e["level"],
                 "Строк": len(e["data"]), "Сумма": e["amount"]} for e in self.errors]
        return pd.DataFrame(rows, columns=["Проверка", "Уровень", "Строк", "Сумма"])
    ...
    def report(self) -> dict:
        if not self.errors:
            return {"status": "ok", "status_label": "Успешно",
                    "total_flags": 0, "total_amount": 0.0,
                    "summary": self.summary_df(), "details": self.details_df()}
        return {"status": "warning", "status_label": "Есть ошибки",
